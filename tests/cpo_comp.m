...
dim=10;
test_result_filename = "test_results\CPO_vs_ACPO_test_results.csv";
convergence_data_folder = "E:\ACPO_Convergence_Curves_Raw\CPO_vs_ACPO";
...
acpo_variants = {'ACPO_afs','ACPO_nmin','ACPO_tsa','ACPO'};
...
for cec=[1,2]
    ...
    for i=1:30
        ...
        fobj = @(x) fhd(x, i);
        fobj_name = benchmark_name + "_F" + num2str(i);
        
        %% Run CPO and record results
        optimizer = "CPO";
        disp(optimizer + " Optimizing " + fobj_name)
        fitnessCPO = zeros(1, RUN_NO);
        Convergence_curve = zeros(RUN_NO, Tmax);
        run_times = zeros(1, RUN_NO);
        parfor j=1:RUN_NO 
            t_start = tic;

            [Best_score,Best_pos,Convergence_curve(j,:)]=CPO(CP_no,Tmax,lb,ub,dim,fobj,opt);
            fitnessCPO(1,j)=Best_score;

            run_times(j) = toc(t_start);
        end
        p = NaN; %this run is the baseline val for rank sum
        resultRow = table(fobj_name, optimizer, mean(fitnessCPO(1,:)), std(fitnessCPO(1,:)), p, mean(run_times));
        ...
        for optim = acpo_variants
            ...
            p = ranksum(fitnessCPO(1,:),fitness(1,:)); 
