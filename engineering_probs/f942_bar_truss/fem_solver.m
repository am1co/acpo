...
% prescribed_dof    = List of prescribed DOF
    ...
    constrained_dofs = 1:(ND * 4); 
    constrained_dofs = constrained_dofs'; % Convert to column vector
    
    % The displacement limit is 15.0 inches (consistent with geometry units)
    u_limit = 15.0; 
    
    % Find the maximum absolute displacement across all constrained DOFs 
    % and all Load Cases.
    u_max_abs = max(max(abs(displm(constrained_dofs,:)))); 
    
    % Calculate the displacement constraint violation (g_disp)
    g_disp = max(u_max_abs - u_limit, 0);
    
    max_stress=max(abs(stress),[],2);
    % Obtain objective and constrained functions
    mass = rho*sum(A.*L);
    f = mass;
    g = [max(max(max_stress)-sigma_a,0), g_disp];
