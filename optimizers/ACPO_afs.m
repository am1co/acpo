...
% % 
function [Gb_Fit,Gb_Sol,Conv_curve]=ACPO_afs(Pop_size,Tmax,lb,ub,dim,fobj,opt)
    arguments
        Pop_size
        Tmax
        lb
    ...
    N_min=min(80, Pop_size); %% Is the minimum population size.
    ...
    while t<=Tmax && Gb_Fit~=opt
        r2=rand;
        local_worst_fitness = max(fitness(1:Pop_size));
        ...
        for i=1:Pop_size
            if rand<rand %% Exploration phase
                ...
                if rand<Tf %% Third defense mechanism
                    ...
                    S=S.*Yt.*Ft;
                    alpha = 1-((fitness(i)-local_worst_fitness) / (Gb_Fit - local_worst_fitness + eps));
